...
import { updateUserModeAction } from "./menu";
...
const applyLoginCallback = (deps: Dependancies) => {
  return () => {
    ...
    return deps.auth.handleRedirectPromise().then((x) => {
      ...
      if (x)
        deps.auth.setActiveAccount(x.account);
      else if (accounts[0])
        deps.auth.setActiveAccount(accounts[0]);
    })
  }
};
...
const ensureUserAuthentication = (deps: Dependancies) =>
  pipe(
    deps.authHelpers.getTokenSilently([deps.env.scopes], deps.authHelpers.getUserAccount()!),
    TE.map((token: AuthenticationResult) => { return verifier.decode(token.accessToken).payload; }),
    TE.chain((accessToken) =>
      pipe(
        ...
        TE.chain((options) =>
          TE.tryCatch(
            () =>
              Promise.resolve(deps.authHelpers.getUserAccount())
                .then((x) => x ?? Promise.reject("account info not found")),
            (x) => x as string
          )
        ),
        TE.map(() => ({
...
export function initEpic(
  ...
  return action$.pipe(
    ...
    filter(() => (!window.location.href.includes("Login") && !window.location.href.includes("Logout") && !window.location.href.includes("Signup") && !window.location.href.includes("LoginError") )),
...
export function loggedInEpic(
  ...
  return action$.pipe(
    ...
    map((x) => {
      return updateUserModeAction(getUserRole(x))
    })
  );
}
...
export const hasUserRole = (requiredRole: UserRolesType, userRole: UserRolesType) => {
  let userRoleHierarchy = Object.values(UserRolesType);
  let validRoles = userRoleHierarchy.splice(0, userRoleHierarchy.findIndex((x) => x === requiredRole) + 1);
  return validRoles.includes(userRole);
}

const getUserRole = (user: User) => {
  if(includes("grant:admin", pathOr([], ["accessToken", "extension_permissions"], user.profile)))
    return UserRolesType.admin;
  else if(includes("grant:customercare", pathOr([], ["accessToken", "extension_permissions"], user.profile)))
    return UserRolesType.customerCare;
  else
    return UserRolesType.user;
};
