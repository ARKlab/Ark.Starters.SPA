...
export const appFetchQuery = (
  ...
  withToaster(withZodResultValidation(withProblemDetails(arkFetchBaseQuery(fetchConfig, appConfig))), {
    toDetails: ({ error }) => {
      const ed = ((): ErrorDetailsType => {
        ...
        if (
          ...
          return {
            title:
              error.status === "TIMEOUT_ERROR" ? t("template:errorHandler.timeout") : t("template:errorHandler.error"),
