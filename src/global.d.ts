 ...
 * Join<["a", "b"], " "> // "a b"
 */
type Join<S extends Stringish[], Sep extends string = ""> = Tail<S>["length"] extends 0
  ? `${S[0]}`
  : `${S[0]}${Sep}${Join<Tail<S>, Sep>}`;

/** Splits a string into an array of substrings. */
type Split<S extends string, Sep extends string = ""> = S extends `${infer U}${Sep}${infer V}`
  ? [U, ...Split<V, Sep>]
  : S extends ""
    ? []
    : [S];
...
type ExactTuple<T, Length extends number, /** don't */ __A extends any[] = []> = __A extends { length: Length }
  ? __A
...
type AtLeastOne<T, Keys extends keyof T = keyof T> = Normalize<
  Pick<T, Exclude<keyof T, Keys>> &
    { [K in Keys]-?: Required<Pick<T, K>> & Partial<Record<Exclude<Keys, K>, T[K]>> }[Keys]
>;
...
type OnlyOneOrNone<T, Keys extends keyof T = keyof T> = Normalize<
  Pick<T, Exclude<keyof T, Keys>> & { [K in Keys]?: Pick<T, K> & Partial<Record<Exclude<Keys, K>, never>> }[Keys]
>;
...
type PickAs<
  T,
  K extends `${string}:${string}` | Exclude<keyof T, K extends `${infer A}:${string}` ? A : never>,
> = ShallowNormalize<
  UnionToIntersection<K extends `${infer A}:${infer B}` ? { [key in B]: T[A] } : never> & Pick<T, Extract<K, keyof T>>
>;
/*`*/
